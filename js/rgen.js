(function () {
      ...
      $(obj)
        ...
        .each(function (index, el) {
          ...
          $(this)
            ...
            .append(
              '<i class="acc-open ' +
                $(obj).attr('data-acc-openclass') +
                ' "></i><i class="acc-close ' +
                $(obj).attr('data-acc-closeclass') +
                '"></i>'
            );
        });
    }
  ...
  rgen.getMultiScripts = function (scripts, path) {
    var self = this;
    return $.when.apply(
      $,
      scripts.map(function (script) {
        // Check if it's a local file and don't add timestamp
        if (script.indexOf('http') !== 0 && script.indexOf('//') !== 0) {
          return $.getScript((path || '') + script);
        } else {
          // For external URLs, you can keep the timestamp if needed
          return $.getScript((path || '') + script + '?_=' + new Date().getTime());
        }
      })
    );
  };
  ...
  rgen.tabs = function (obj) {
    ...
    if ($(obj.tb).hasClass('tabs-auto')) {
      var t = 0,
        tb_activeClass = $(obj.tb).attr('data-tb-active')
          ? 'active ' + $(obj.tb).attr('data-tb-active')
          : 'active',
        pn_activeClass = $(obj.tb).attr('data-pn-active')
          ? 'active ' + $(obj.tb).attr('data-pn-active')
          : 'active',
      ...
      $(obj.tb).on('click', '.tb-list > .tb', function (e) {
        ...
        $(this)
          .closest('.tb-list')
          .find('.tb')
          .removeClass(tb_activeClass)
          .addClass(tb_normalClass);
      ...
      $(obj)
        .on('click', function (e) {
          ...
          var tb_activeClass = $(obj).attr('data-tb-active')
              ? 'active ' + $(obj).attr('data-tb-active')
              : 'active',
            pn_activeClass = $(obj).attr('data-pn-active')
              ? 'active ' + $(obj).attr('data-pn-active')
              : 'active',
  ...
  rgen.linkscroll = function (obj) {
    ...
    $(document).on('click', obj, function (e) {
      e.preventDefault();
      if (
        $(this).closest('.nav-links').hasClass('nav-links') == false &&
        $(this).attr('href').indexOf('popup') === -1
      ) {
        // target element id
  ...
  rgen.filter = function (obj) {
    ...
    list.find('.active')
      ? list.find('.active').trigger('click')
      : list.find('[data-filter]').first().trigger('click');
  };
  ...
  jQuery(document).ready(function ($) {
    ...
    $o.navlink =
      $('.nav-wrp').find('.nav-links').length > 0 && $o.r ? $o.navwrp.find('.nav-links') : false;
    ...
    $o.popgallerywidget =
      $('.popgallery-widget').length > 0 && $o.r ? $('.popgallery-widget') : false;
    ...
    if ($o.r) {
      ...
      if ($o.navlink) {
        ...
        $o.navlink.find('a').smoothScroll({
          speed: 1200,
          offset:
            $o.navwrp.attr('data-sticky') == 'y' || $o.navwrp.attr('data-sticky-scroll') == 'y'
              ? -($o.navwrp.height() - 20)
              : 0,
      ...
      if ($o.masonry) {
        for (var i = 0; i < $o.masonry.length; i++) {
          var gridGt = $($o.masonry[i]).attr('data-grid-gutter')
              ? $($o.masonry[i]).attr('data-grid-gutter') + 'px'
              : '10px',
            gridCls = $($o.masonry[i]).attr('data-grid-item')
              ? $($o.masonry[i]).attr('data-grid-item')
              : '.masonry-item';
      ...
      if ($o.gradient) {
        for (var i = 0; i < $o.gradient.length; i++) {
          ...
          $($o.gradient[i]).css({
            ...
            background:
              'linear-gradient(to bottom, ' + grd_color[0] + ' 0%, ' + grd_color[1] + ' 100%)',
      ...
      if (rgen.elcheck($o.bgslider)) {
        var vages_script_arr = ['lib/vegas/vegas.min.js'];
      ...
      if ($o.socialsection) {
        ...
        rgen.getMultiScripts(social_script_arr, 'lib/social-feed/index.html').done(function () {
          // TWITTER
          if ($o.twitterwidget) {
